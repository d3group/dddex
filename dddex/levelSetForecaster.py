...
class LevelSetForecaster(BasePredictor):
    """
    `LevelSetForecaster` turns any point predictor that has a .predict-method 
    ...
    In the case of the standard `LevelSetForecaster` defined here, samples are grouped
    ...
    NOTE 1: `LevelSetForecaster` can only be applied to estimators that have been 
    fitted already.
    NOTE 2: `LevelSetForecaster` can only be defined for point predictors with a 
...
class LevelSetForecaster_kNN(BasePredictor):
    """
     `LevelSetForecaster_kNN` turns any point predictor that has a .predict-method 
    into an estimator of the condititional density of the underlying distribution.
    The basic idea of each level-set based approach is to interprete the point forecast
    generated by the underlying point predictor as a similarity measure of samples.
    In the case of the `LevelSetForecaster_kNN` defined here, for every new samples
    ...
    NOTE 1: The `LevelSetForecaster_kNN` class can only be applied to estimators that 
    have been fitted already.
    NOTE 2: In contrast to the standard `LevelSetForecaster`, it is possible to apply
    `LevelSetForecaster_kNN` to arbitrary dimensional point predictors.
    """
