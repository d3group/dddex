...
class LevelSetKDEx(BasePredictor):
    ...
    In the case of the standard `LevelSetKDEx` defined here, samples are grouped
    ...
    NOTE 1: `LevelSetKDEx` can only be applied to estimators that have been 
    fitted already.
    
    NOTE 2: `LevelSetKDEx` can only be defined for point predictors with a 
