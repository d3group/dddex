...
# %% auto 0
__all__ = ['LevelSetKDEx_multivariate', 'LevelSetKDEx_multivariate2']
...
class LevelSetKDEx_multivariate(BaseWeightsBasedEstimator_multivariate, BaseLSx):
    ...
    def fit(self: LevelSetKDEx, 
        ...
        # clustersZero = binSizes[np.where(binSizes == 0)[0]]
        ...
        clustersTooSmall = np.array(binSizes.index[np.where(binSizes < threshold)[0]])
        ...
        for i, clusterTooSmall in enumerate(clustersTooSmall):
            ...
            if binSizes[clusterTooSmall] >= threshold:
                continue
            ...
            binSizes[clusterToMerge] += binSizes[clusterTooSmall]
            binSizes[clusterTooSmall] = 0
...
# %% ../nbs/02_levelSetKDEx_multivariate.ipynb 7
class LevelSetKDEx_multivariate2(BaseWeightsBasedEstimator_multivariate, BaseLSx):
    """
    `LevelSetKDEx` turns any point forecasting model into an estimator of the underlying conditional density.
    The name 'LevelSet' stems from the fact that this approach interprets the values of the point forecasts
    as a similarity measure between samples. The point forecasts of the training samples are sorted and 
    ...
    def fit(self: LevelSetKDEx, 
        ...
        if len(binsTooSmall) > 0:
            ...
            indicesPerBin = {binIndex: np.array(indices) for binIndex, indices in indicesPerBin.items() if binIndex not in binsTooSmall}
            binSizes = {binIndex: len(indices) for binIndex, indices in indicesPerBin.items() if binIndex not in binsTooSmall}
            binSizes = pd.Series(binSizes)
