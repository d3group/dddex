...
# %% auto 0
__all__ = ['LevelSetKDE', 'generateBins', 'LevelSetKED_kNN', 'binSizeCV', 'scoresForFold', 'getCoefPres']

# %% ../nbs/01_levelSetForecaster.ipynb 7
class LevelSetKDE(BasePredictor):
    """
    `LevelSetKED` turns any point predictor that has a .predict-method 
    ...
    In the case of the standard `LevelSetKED` defined here, samples are grouped
    according to their point predictions with each resulting bin consisting of 'binSize'-
    many samples.
    The resulting empirical distribution of the training samples inside each bin is 
    viewed as our estimation of the conditional distribution of each sample whose
    point forecast lies inside the boundaries of the respective bin.    
    
    NOTE 1: `LevelSetKED` can only be applied to estimators that have been 
    fitted already.
    NOTE 2: `LevelSetKED` can only be defined for point predictors with a 
    univariate target variable 'y'.
    """
    ...
    def __str__(self):
        return f"LevelSetKED(estimator = {self.estimator}, binSize = {self.binSize})"
    __repr__ = __str__      
...
@patch
def getWeights(self: LevelSetKED, 
...
class LevelSetKED_kNN(BasePredictor):
    """
     `LevelSetKED_kNN` turns any point predictor that has a .predict-method 
    into an estimator of the condititional density of the underlying distribution.
    The basic idea of each level-set based approach is to interprete the point forecast
    generated by the underlying point predictor as a similarity measure of samples.
    In the case of the `LevelSetKED_kNN` defined here, for every new samples
    ...
    NOTE 1: The `LevelSetKED_kNN` class can only be applied to estimators that 
    have been fitted already.
    NOTE 2: In contrast to the standard `LevelSetKED`, it is possible to apply
    `LevelSetKED_kNN` to arbitrary dimensional point predictors.
    """
    ...
    def __str__(self):
        return f"LevelSetKED_kNN(estimator = {self.estimator}, binSize = {self.binSize})"
    __repr__ = __str__   
      

# %% ../nbs/01_levelSetForecaster.ipynb 18
@patch 
def fit(self:LevelSetKED_kNN, 
        X: np.ndarray, # Feature matrix used by 'estimator' to predict 'y'.
...
@patch
def getWeights(self: LevelSetKED_kNN, 
               X: np.ndarray, # Feature matrix of samples for which conditional density estimates are computed.
               outputType: 'all' | # Specifies structure of output.
                           'onlyPositiveWeights' | 
                           'summarized' | 
...
class binSizeCV:
    ...
    def __init__(self,
        ...
        if isinstance(estimator, (LevelSetKED, LevelSetKED_kNN)):
            raise ValueError("'estimator' has to be a point predictor and not a LevelSetKED-Object!")   
    ...
    if self.refitPerProb:
        ...
        for binSize in binSizeBestPerProb.unique():
            ...
            if self.LSF_type == 'LSF':
                LSF = LevelSetKED(estimator = self.estimator, 
                                         binSize = binSize)
            else:
                LSF = LevelSetKED_kNN(estimator = self.estimator, 
                                             binSize = binSize)
...
def scoresForFold(cvFold, binSizeGrid, probs, estimator, LSF_type, y, X):
    ...
    for binSize in iter(binSizeGrid):
        ...
        if LSF_type == 'LSF':
            estimatorLSF = LevelSetKED(estimator = estimator,
                                              binSize = binSize)
        else:
            estimatorLSF = LevelSetKED_kNN(estimator = estimator,
                                                  binSize = binSize)
