# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/01_levelSetKDE.ipynb.

# %% ../nbs/01_levelSetKDE.ipynb 5
...
# %% auto 0
__all__ = ['LevelSetKDE', 'generateBins', 'LevelSetKDE_kNN', 'binSizeCV', 'scoresForFold', 'getCoefPres']

# %% ../nbs/01_levelSetKDE.ipynb 7
class LevelSetKDE(BasePredictor):
    """
    `LevelSetKDE` turns any point predictor that has a .predict-method 
    ...
    In the case of the standard `LevelSetKDE` defined here, samples are grouped
    ...
    NOTE 1: `LevelSetKDE` can only be applied to estimators that have been 
    fitted already.
    NOTE 2: `LevelSetKDE` can only be defined for point predictors with a 
...
class LevelSetKDE_kNN(BasePredictor):
    ...
    The basic idea of each level-set based approach is to interprete the point forecast
    generated by the underlying point predictor as a similarity measure of samples.
    In the case of the `LevelSetKDE_kNN` defined here, for every new samples
...
class binSizeCV:
    ...
    def __init__(self,
        ...
        if isinstance(estimator, (LevelSetKDE, LevelSetKDE_kNN)):
            raise ValueError("'estimator' has to be a point predictor and not a LevelSetKDE-Object!")   
    ...
    if self.refitPerProb:
        ...
        for binSize in binSizeBestPerProb.unique():
            ...
            if self.LSF_type == 'LSF':
                LSF = LevelSetKDE(estimator = self.estimator, 
                                         binSize = binSize)
            else:
                LSF = LevelSetKDE_kNN(estimator = self.estimator, 
                                             binSize = binSize)
...
def scoresForFold(cvFold, binSizeGrid, probs, estimator, LSF_type, y, X):
    ...
    for binSize in iter(binSizeGrid):
        ...
        if LSF_type == 'LSF':
            estimatorLSF = LevelSetKDE(estimator = estimator,
                                              binSize = binSize)
        else:
            estimatorLSF = LevelSetKDE_kNN(estimator = estimator,
                                                  binSize = binSize)
